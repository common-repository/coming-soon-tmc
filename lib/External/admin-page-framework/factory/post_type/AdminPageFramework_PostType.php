...
abstract class cs_tmc_AdminPageFramework_PostType_Router extends cs_tmc_AdminPageFramework_Factory {
...
abstract class cs_tmc_AdminPageFramework_PostType_Model extends cs_tmc_AdminPageFramework_PostType_Router {
    public function __construct($oProp) {
        parent::__construct($oProp);
        ...
        if ($this->oProp->bIsAdmin) {
            ...
            if ($this->oProp->sCallerPath) {
                new cs_tmc_AdminPageFramework_PostType_Model__FlushRewriteRules($this);
            }
        }
    }
    ...
    public function _replyToRegisterPostType() {
        ...
        new cs_tmc_AdminPageFramework_PostType_Model__SubMenuOrder($this);
    }
...
abstract class cs_tmc_AdminPageFramework_PostType_View extends cs_tmc_AdminPageFramework_PostType_Model {
    public function __construct($oProp) {
        parent::__construct($oProp);
        if ($this->oProp->bIsAdmin) {
    ...
    public function _replyToPrintStyle() {
        ...
        if (!empty($this->oProp->sStyle)) {
            echo "<style type='text/css' id='cs-tmc-style-post-type'>" . $this->oProp->sStyle . "</style>";
        }
    }
...
abstract class cs_tmc_AdminPageFramework_PostType_Controller extends cs_tmc_AdminPageFramework_PostType_View {
...
abstract class cs_tmc_AdminPageFramework_PostType extends cs_tmc_AdminPageFramework_PostType_Controller {
    protected $_sStructureType = 'post_type';
    public function __construct($sPostType, $aArguments = array(), $sCallerPath = null, $sTextDomain = 'cs-tmc') {
        if (empty($sPostType)) {
            return;
        }
        ...
        $this->oProp->sPostType = cs_tmc_AdminPageFramework_WPUtility::sanitizeSlug($sPostType);
        $this->oProp->aPostTypeArgs = $aArguments;
    ...
    private function _getCallerScriptPath($sCallerPath) {
        ...
        $_sPageNow = cs_tmc_AdminPageFramework_Utility::getElement($GLOBALS, 'pagenow');
        if (in_array($_sPageNow, array('edit.php', 'post.php', 'post-new.php', 'plugins.php', 'tags.php', 'edit-tags.php', 'term.php'))) {
            return cs_tmc_AdminPageFramework_Utility::getCallerScriptPath(__FILE__);
        }
        return null;
    }
}
